...
maze_cell** parse_maze( FILE* maze_file, int dimension )
{
  ...
  for ( row = 0; row < dimension; ++row ) {
    // INSERT CODE HERE (1 line)
	  maze[row] = (maze_cell*)calloc(dimension, dimension*sizeof(maze_cell));
  }
...
void generate_all_paths( char*** pathsetref, int* numpathsref, maze_cell** maze, int dimension, int row, int column, char* path )
{
  ...
  if ( // INSERT CODE HERE: Simply return if we hit one of the base cases (there are more than 1)
       ...
       row < 0 || column < 0 || row > dimension-1 || column > dimension-1 || maze[row][column].visited == VISITED || maze[row][column].character == '*') {
    return;
	}
...
void construct_shortest_path_info ( char** pathset, int numpaths, char* outputbuffer )
{
	// INSERT CODE HERE
	outputbuffer[0] = '\0';
	char shortest_path[100];
	strcpy(shortest_path, *(pathset));
	int length = strlen(*(pathset));

	for (int i = 0; i < numpaths; i++) {
		int temp = strlen(*(pathset + i));
		if (temp < length) {
			length = temp;
			shortest_path[100] = *(pathset + i);
		}
	}

	int shortest_length = strlen(shortest_path);

	char begin[] = "Shortest path: ";
	strcpy(outputbuffer, begin);
	strcat(outputbuffer, shortest_path);
}
...
void construct_cheapest_path_info ( char** pathset, int numpaths, char* outputbuffer )
{
	// INSERT CODE HERE
	outputbuffer[0] = '\0';
	char cheapest_path[100];
	strcpy(cheapest_path, *(pathset));
	int cost1 = path_cost(*(pathset));

	for (int i = 0; i < numpaths; i++) {
		int cost2 = path_cost(*(pathset + i));
		if ( cost2 < cost1) {
			strcpy(cheapest_path,*(pathset + i));
			cost1 = cost2;
		}
	}

	int cheapest_length = strlen(cheapest_path);
	char begin[] = "Cheapest path: ";
	strncat(outputbuffer, begin, 15);
	strncat(outputbuffer, cheapest_path, cheapest_length);
	strcat(outputbuffer, "\n");
	char secondline[] = "Cheapest path cost: ";
	strcat(outputbuffer, secondline);
	char cost_buffer[100];
	sprintf(cost_buffer, "%d", cost1);
